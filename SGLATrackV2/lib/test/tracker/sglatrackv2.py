import math
import numpy as np
from lib.models.sglatrackv2 import build_sglatrackv2
...
class SGLATrackV2(BaseTracker):
    def __init__(self, params, data):
        super(SGLATrackV2, self).__init__(params)
        network = build_sglatrackv2(params.cfg, training=False)
    ...
    def track(self, image, info: dict = None):
        ...
        with torch.no_grad():
            out_dict, aux_dict = self.network.forward_test(self.z, search=[search.tensors], mode='zx', ce_template_mask=None, hlist = self.hlist )
        ...
        #     self.hlist = torch.cat( (self.hlist,aux_dict['hlist']) , dim=1 )
        # elif self.frame_id > 3 and self.frame_id % self.inter == 0:
        #     hlist_update = self.hlist[:,self.hnum:,:]
        #     self.hlist = torch.cat( (hlist_update,aux_dict['hlist']) , dim=1 )
