...
from lib.models.sglatrackv2.base_backbone import BaseBackbone
...
class MixedCrossAttention(Attention):
    def forward(self, x, len_t,returnhlist=False):
        B, N, C = x.shape
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
        ...
        self.attn = MixedCrossAttention(dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop, attn_type=attn_type)
    ...
    def forward(self, x, len_t,returnhlist=False):
        ...
        if returnhlist==False:
            y= self.attn(self.norm1(
                torch.cat((x_t, x_s), dim=1)), len_t,returnhlist)
            y_s = y[:, len_t:]
            ...
            y, attn_s2ts= self.attn(self.norm1(
                torch.cat((x_t, x_s), dim=1)), len_t,returnhlist)
            attn_s2t = attn_s2ts[:,:,:,:len_t].detach()
            avg = attn_s2t.mean(dim=3).mean(dim=1)
            sorted_attn, indices = torch.sort(avg, dim=1, descending=True)
            ...
            hlist = x_s.gather(dim=1, index=indices[:,:6].unsqueeze(-1).expand(x_s.shape[0], -1, x_s.shape[-1]))
            x = torch.cat((x_t, x_s), dim=1)
            return x, hlist
