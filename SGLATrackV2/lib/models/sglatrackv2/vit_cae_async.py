...
class ThreeLayerMLP(nn.Module):
    def __init__(self, input_dim=256, output_dim=6, hidden_dim=128):
        ...
        self.fc1 = nn.Linear(input_dim, hidden_dim)  # 第一层：输入维度320，输出维度160
        self.fc2 = nn.Linear(hidden_dim, output_dim)  # 输出层：隐藏层160 -> 输出11
...
class VIT_Backbone(nn.Module):
    ...
    def finetune_track(self, cfg, patch_start_index=1):
        ...
        self.MLP = ThreeLayerMLP(input_dim=256, output_dim=12-1-start_layer)
    ...
    def forward(self, z, x=None,
        ...
        if mode is None:
            ...
            x_feat = self.forward_zx(z_feat, x)
        elif mode == 'zx':
            x_feat, aux_dict = self.forward_zx(z, x,hlist)
        elif mode == 'z':
            x_feat, aux_dict  = self.forward_z(z)


        return x_feat, aux_dict
    ...
    if pretrained:
        if 'npz' in pretrained:
            ...
            state_dict = torch.load(
                '/public/workspace/xcc23/ASTrack_t6i6_caev2S_MLP5_3s/pretrained_networks/caev2_small_300ep.pth', map_location="cpu")
...
def CAE_Tiny_patch16_224_Async(pretrained=False, **kwargs) -> VIT_Backbone:
    model_kwargs = dict(mlp_ratio=4, qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6),
                        patch_size=16, embed_dim=192, num_heads=12,
                        drop_rate=0., attn_drop_rate=0., init_values=0.1,
                        use_abs_pos_emb=True, **kwargs)
    model = _create_ViT_CAE(pretrained=pretrained, **model_kwargs)
    # model.adpat4track()
...
def CAE_Small_patch16_224_Async(pretrained=False, **kwargs) -> VIT_Backbone:
    model_kwargs = dict(mlp_ratio=4, qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6),
                        patch_size=16, embed_dim=384, num_heads=12,
                        drop_rate=0., attn_drop_rate=0., init_values=0.1,
                        use_abs_pos_emb=True, **kwargs)
    model = _create_ViT_CAE(pretrained=pretrained, **model_kwargs)
    # model.adpat4track()
