...
from lib.train.actors import SGLATrackV2Actor
# for import modules
import importlib
...
def run(settings):
    ...
    # Create network
    if settings.script_name == "sglatrackv2":
        net = build_sglatrackv2(cfg)
    else:
    ...
    if settings.script_name == "sglatrackv2":
        ...
        actor = SGLATrackV2Actor(net=net, objective=objective, loss_weight=loss_weight, settings=settings, cfg=cfg)
    else:
        raise ValueError("illegal script name")
